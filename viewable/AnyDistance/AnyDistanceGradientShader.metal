...
struct VertexIn { float2 position; };

vertex VertexOut gradient_animation_vertex(const device VertexIn* verts [[buffer(0)]],
    ...
    // Convert pixel coords to NDC (0..w,0..h) => (-1..1)
    float2 ndc = (pos / viewSize) * 2.0 - 1.0;
    ndc.y = -ndc.y; // flip Y for Metal
    out.position = float4(ndc, 0, 1);
    out.time = time + (float)page * 10.0;
